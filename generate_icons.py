...
os.makedirs("IcoGeneratorOutput")
				...
				if object["Class"] == "IcoGenerator":
					...
					for image in object["Images"]:
						...
						try:
							...
							ibase = Image.open(our_path + "/../icons/" + image["Base"] + ".png")
							pixels = ibase.load()
							...
							if "MulMask" in image:
								try:
									mbase = Image.open(our_path + "/../icons/" + image["MulMask"] + ".png")
									mpixels = mbase.load()
									for i in range(img.size[0]):
										for j in range(img.size[1]):
								    ...
								    print (our_path + "/../icons/" + image["MulMask"] + ".png is missed; Base image created.")

							if "AddMask" in image:
								try:
									if isinstance(image["AddMask"], list):
										for addmask in image["AddMask"]:
											mbase = Image.open(our_path + "/../icons/" + addmask + ".png")
											mpixels = mbase.load()
											for i in range(img.size[0]):
												for j in range(img.size[1]):
										...
										mbase = Image.open(our_path + "/../icons/" + image["AddMask"] + ".png")
										mpixels = mbase.load()
										for i in range(img.size[0]):
											for j in range(img.size[1]):
												r = int(lerp(output[i, j][0], mpixels[i, j][0], mpixels[i, j][3] / 255.0))
							...
							img.save("IcoGeneratorOutput/" + image["NewName"] + ".png")
						except IOError:
							print (image["NewName"] + " err" + str(IOError))

for dirpath, dirnames, filenames in os.walk(our_path + "/../icons"):
	for filename in [f for f in filenames if f.endswith(".png")]:
		fullname = os.path.join(dirpath, filename)
		img = Image.open(fullname)
		name = "IcoGeneratorOutput/" + os.path.splitext(filename)[0] + ".png"
		img.save(name)
