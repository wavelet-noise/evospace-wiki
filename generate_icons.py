				...
				if object["Class"] == "IcoGenerator":
					...
					for image in object["Images"]:
						...
						try:
							...
							ibase = Image.open("IcoGenerator/" + image["Base"] + ".png")
							pixels = ibase.load()
							...
							if "MulMask" in image:
								try:
									mbase = Image.open("IcoGenerator/" + image["MulMask"] + ".png")
									mpixels = mbase.load()
									for i in range(img.size[0]):
										for j in range(img.size[1]):
								    ...
								    print ("IcoGenerator/" + image["MulMask"] + ".png is missed; Base image created.")

							if "AddMask" in image:
								try:
									if isinstance(image["AddMask"], list):
										for addmask in image["AddMask"]:
											mbase = Image.open("IcoGenerator/" + addmask + ".png")
											mpixels = mbase.load()
											for i in range(img.size[0]):
												for j in range(img.size[1]):
										...
										mbase = Image.open("IcoGenerator/" + image["AddMask"] + ".png")
										mpixels = mbase.load()
										for i in range(img.size[0]):
											for j in range(img.size[1]):
												r = int(lerp(output[i, j][0], mpixels[i, j][0], mpixels[i, j][3] / 255.0))
...
for dirpath, dirnames, filenames in os.walk("IcoGenerator"):
